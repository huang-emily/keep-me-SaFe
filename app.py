...
def get_coord(address, country):
    ...
    default_json = json.load(response)
...
def graph(hour, dow, month, year, severe):
    ...
    fig = px.scatter_map(df, lat="tb_latitude", lon="tb_longitude", 
...
def verify_route(origin, destination, user_dt):
    ...
    parameters = {'origin': origin, 'destination': destination, 'key': API_KEY, 'mode': 'walking'}
...
def create_graph(location):
    ...
    fig = px.scatter_map(inputted_point, 
                            lat="latitude", 
                            lon="longitude", 
                            map_style='basic',
                            zoom=12)
